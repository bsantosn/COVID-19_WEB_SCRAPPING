...
class Coronavirus:
    ...
    # Cáculo de nuevos campos para el dataframe -- Posibilidad de añadir más campos
...
class Gdp:
    ...
    def scrapping_gdp(self):
        ...
        colum = ["Country", "Year_GDP_update", "GDP_Annual", "GDP_Var1", "GDP_Var2", "GDP_Var3", "GDP_Var4", "GDP_Var5"]
        list = []
        final = []
        for row in rows:
            for item in row.find_all('td'):
...
class InfoCovid(Coronavirus, Vacunas, Gdp):

    # constructor
    def __init__(self, covid, vaccine, gdp):
        self.covid = covid
    ...
    def datos_finales(self):
        ...
        df_inner = self.gdp.merge(self.covid, on='Country', how='inner').merge(self.vaccine, on='Country', how='inner')
...
def export_csv(datos):
    ...
    datos.to_csv("Data/covid_19.csv", index=False)
    return "OK"
